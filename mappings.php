...
$syncenabled = optional_param('sync_enabled', 1, PARAM_BOOL);
...
if ($save && $courseid && $subjectid && $aycode) {
    ...
    redirect(new moodle_url('/local/schooleescore_bridge/mappings.php'), get_string('mapping_saved', 'local_schooleescore_bridge'));
}

$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/local/schooleescore_bridge/mappings.php'));
...
$formfields[] = html_writer::empty_tag('input', ['type' => 'text', 'name' => 'subjectid', 'id' => 'id_subjectid', 'required' => 'required']);
$formfields[] = html_writer::empty_tag('br');
...
$formfields[] = html_writer::empty_tag('input', ['type' => 'text', 'name' => 'academic_year_code', 'id' => 'id_aycode', 'required' => 'required']);
$formfields[] = html_writer::empty_tag('br');
...
echo html_writer::tag('form', implode('', $formfields), ['method' => 'post', 'action' => new moodle_url('/local/schooleescore_bridge/mappings.php')]);
